...
describe('utils', () => {
	describe('trim', () => {
		it('trim_start', () => {
			...
			assert.equal(value, 'poly content \r\n\t	');
		});
	...
	describe('get_name_from_filename', () => {
		it('uses the basename', () => {
			assert.equal(get_name_from_filename('path/to/Widget.poly'), 'Widget');
		});

		it('uses the directory name, if basename is index', () => {
			assert.equal(get_name_from_filename('path/to/Widget/index.poly'), 'Widget');
		});

		it('handles Windows filenames', () => {
			assert.equal(get_name_from_filename('path\\to\\Widget.poly'), 'Widget');
		});

		it('handles special characters in filenames', () => {
			assert.equal(get_name_from_filename('@.poly'), '_');
			assert.equal(get_name_from_filename('&.poly'), '_');
			assert.equal(get_name_from_filename('~.poly'), '_');
		});
	});
