...
import { clear_loops, flush, set_now, set_raf } from 'poly/internal';
...
async function run_test(dir) {
	...
	it_fn.each`
		...
		async function test() {
			...
			mod = await load('./main.poly');
			PolyComponent = mod.default;

			// Put things we need on window for testing
			// @ts-expect-error
			window.PolyComponent = PolyComponent;
			...
			if (hydrate && from_ssr_html) {
				...
				const SsrPolyComponent = (await load_ssr('./main.poly')).default;
				const { html } = SsrPolyComponent.render(config.props);
			...
			const component = new PolyComponent(options);
		...
		await test()
			.catch((err) => {
				if (config.error && !unintended_error) {
					...
					for (const file of glob('**/*.poly', { cwd })) {
						...
						const out = `${dir}/${file.replace(/\.poly$/, '.js')}`;
			...
			.catch((err) => {
				...
				err.stack += `\n\ncmd-click: ${path.relative(process.cwd(), cwd)}/main.poly`;
