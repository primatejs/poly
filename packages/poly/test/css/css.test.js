...
import * as poly from 'poly/compiler';
...
describe('css', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach((dir) => {
		...
		it_fn(dir, async () => {
			...
			const input = fs
				.readFileSync(`${cwd}/input.poly`, 'utf-8')
				.replace(/\s+$/, '')
				.replace(/\r/g, '');

			const expected_warnings = (config.warnings || []).map(normalize_warning);

			const dom = poly.compile(input, Object.assign({}, config.compileOptions || {}));

			const ssr = poly.compile(input, Object.assign({}, config.compileOptions || {}));
			...
			try {
				ClientComponent = (await load('input.poly')).default;
			} catch (err) {
			...
			try {
				ServerComponent = (await load_ssr('input.poly')).default;
			} catch (err) {
				console.log(dom.js.code);
				throw err;
			}
...
function replace_css_hash(str) {
	return str.replace(/poly-[a-z0-9]+/g, 'poly-xyz');
}
