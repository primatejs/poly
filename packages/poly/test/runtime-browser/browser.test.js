...
async function run_browser_test(dir) {
	...
	it_fn.each([false, true])(`${dir} hydrate: %s`, async (hydrate) => {
		...
		const build_result = await build({
			...
			write: false,
			alias: {
				__MAIN_DOT_SVELTE__: path.resolve(__dirname, 'samples', dir, 'main.poly'),
			...
			plugins: [
				{
					...
					setup(build) {
						build.onLoad({ filter: /\.poly$/ }, ({ path }) => {
							const compiled = poly.compile(fs.readFileSync(path, 'utf-8').replace(/\r/g, ''), {
