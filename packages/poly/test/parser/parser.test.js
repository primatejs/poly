...
describe('parse', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach((dir) => {
		...
		const skip = !fs.existsSync(`${__dirname}/samples/${dir}/input.poly`);
		if (skip) {
			console.warn(
				`skipping ${dir} because no input.poly exists. This could be a leftover folder from a different branch.`
			);
		}
		...
		it_fn(dir, () => {
			...
			const input = fs
				.readFileSync(`${__dirname}/samples/${dir}/input.poly`, 'utf-8')
				.trimEnd()
				.replace(/\r/g, '');
