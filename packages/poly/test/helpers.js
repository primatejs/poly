...
const poly_path = fileURLToPath(new URL('..', import.meta.url).href).replace(/\\/g, '/');
	...
	async function load(file) {
		...
		if (file.endsWith('.poly')) {
			...
			const __import = (id) => {
				...
				if (id === 'poly') {
					resolved = `${poly_path}src/runtime/index.js`;
				}

				if (id.startsWith('poly/')) {
					resolved = `${poly_path}src/runtime/${id.slice('poly'.length)}/index.js`;
				}
			...
			// any imported Poly components as well. A few edge cases aren't handled but also
