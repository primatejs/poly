 ...
 * @param {Window} window
 * @param {string} html
 * @param {{ removeDataPoly?: boolean, preserveComments?: boolean }} param2
 */
export function normalize_html(
	window,
	html,
	{ removeDataPoly = false, preserveComments = false }
) {
	try {
		const node = window.document.createElement('div');
		node.innerHTML = html
			.replace(/(<!--.*?-->)/g, preserveComments ? '$1' : '')
			.replace(/(data-poly-h="[^"]+")/g, removeDataPoly ? '' : '$1')
			.replace(/>[ \t\n\r\f]+</g, '><')
			.trim();
		clean_children(node);
...
/**
 * @param {{ removeDataPoly?: boolean }} options
 */
