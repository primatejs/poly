...
describe('sourcemaps', async () => {
	...
	async function run(dir) {
		...
		it_fn(dir, async () => {
			...
			const input_file = path.resolve(`${__dirname}/samples/${dir}/input.poly`);
			...
			const preprocessed = await poly.preprocess(
				input.code,
				config.preprocess || {},
				config.options || {
					filename: 'input.poly'
			...
			const { js, css } = poly.compile(preprocessed.code, {
				filename: 'input.poly',
			...
			js.code = js.code.replace(/generated by Poly v\d+\.\d+\.\d+/, (match) =>
				match.replace(/\d/g, 'x')
			);

			fs.writeFileSync(`${output_base}.poly`, preprocessed.code);
			if (preprocessed.map) {
				fs.writeFileSync(
					`${output_base}.poly.map`,
					// TODO encode mappings for output - poly.preprocess returns decoded mappings
			...
			if (js.map) {
				assert.deepEqual(
					...
					(config.js_map_sources || ['input.poly']).sort(),
			...
			if (css.map) {
				assert.deepEqual(
					...
					(config.css_map_sources || ['input.poly']).sort(),
