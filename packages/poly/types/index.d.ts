declare module 'poly' {
   ...
   * ```
   */
  export type ComponentEvents<Component extends PolyComponent_1> =
	Component extends PolyComponent<any, infer Events> ? Events : never;
   ...
   * ```
   */
  export type ComponentProps<Component extends PolyComponent_1> =
	Component extends PolyComponent<infer Props> ? Props : never;
   ...
   * ```
   */
  export type ComponentType<Component extends PolyComponent = PolyComponent> = (new (
	options: ComponentConstructorOptions<
	  Component extends PolyComponent<infer Props> ? Props : Record<string, any>
	>
  ) => Component) & {
  ...
  interface EventDispatcher<EventMap extends Record<string, any>> {
	...
	/**
	 * Base class for Poly components. Used when dev=false.
	 *
	 * 
	 */
	class PolyComponent_1<Props extends Record<string, any> = any, Events extends Record<string, any> = any> {
		/**
		 * ### PRIVATE API
		 *
	 ...
	 * 
	 */
	export class PolyComponent<Props extends Record<string, any> = any, Events extends Record<string, any> = any, Slots extends Record<string, any> = any> extends PolyComponent_1<Props, Events> { [prop: string]: any;
	 ...
	 * 
	 */
	export class PolyComponentTyped<Props extends Record<string, any> = any, Events extends Record<string, any> = any, Slots extends Record<string, any> = any> extends PolyComponent<Props, Events, Slots> {
	}
	/**
...
declare module 'poly/compiler' {
  ...
  export interface CompileOptions {
	...
	/**
	 * If `"dom"`, Poly emits a JavaScript class for mounting to the DOM.
	 * If `"ssr"`, Poly emits an object with a `render` method suitable for server-side rendering.
	...
	/**
	 * If `"throw"`, Poly throws when a compilation error occurred.
	 * If `"warn"`, Poly will treat errors as warnings and add them to the warning report.
	 *
	 * @default 'throw'
	 */
	...
	/**
	 * If `"strict"`, Poly returns a variables report with only variables that are not globals nor internals.
	 * If `"full"`, Poly returns a variables report with all detected variables.
	...
	/**
	 * If `true`, Poly generate sourcemaps for components.
	...
	/**
	 * The location of the `poly` package.
	 * Any imports from `poly` or `poly/[module]` will be modified accordingly.
	 *
	 * @default 'poly'
	 */
	polyPath?: string;
	...
	/**
	 * If `true`, tells the compiler to generate a custom element constructor instead of a regular Poly component.
	 *
	 * @default false
	 */
	...
	/**
	 * A `string` that tells Poly what tag name to register the custom element with.
	 ...
	 * - `'external'` (formerly `false`), the CSS will be returned in the `css` field of the compilation result. Most Poly bundler plugins will set this to `'external'` and use the CSS that is statically generated for better performance, as it will result in smaller JavaScript bundles and the output can be served as cacheable `.css` files.
	 * - `'none'`, styles are completely avoided and no CSS output is generated.
	 */
	css?: 'injected' | 'external' | 'none' | boolean;

	/**
	 * A `number` that tells Poly to break the loop if it blocks the thread for more than `loopGuardTimeout` ms.
	...
	/**
	 *  If `true`, exposes the Poly major version in the browser by adding it to a `Set` stored in the global `window.__poly.v`.
	 *
	 * @default true
	 */
  ...
  /** The returned shape of `compile` from `poly/compiler` */
  export interface CompileResult {
	...
	vars: Var[];
	/** An object used by the Poly developer team for diagnosing the compiler. Avoid relying on it to stay the same! */
	stats: {
	  timings: {
		total: number;
	  };
  ...
  export type MarkupPreprocessor = (options: {
	/**
	 * The whole Poly file content
	 */
	content: string;
	/**
	 * The filename of the Poly file
	 */
	filename?: string;
  ...
  /**
   * A preprocessor group is a set of preprocessors that are applied to a Poly file.
   */
   ...
   */
  export interface PolyPreprocessor<
...
declare module 'poly/types/compiler/preprocess' {
  /**
   * The result of a preprocessor run. If the preprocessor does not return a result, it is assumed that the code is unchanged.
   */
  export interface Processed {
...
declare module 'poly/types/compiler/interfaces' {
	import type { AssignmentExpression, Node, Program } from 'estree';
	import type { SourceMap } from 'magic-string';
  interface BaseNode {
	start: number;
...
declare module 'poly/action' {
  /**
   * Actions can return an object containing the two properties defined in this interface. Both are optional.
   * - update: An action can have a parameter. This method will be called whenever that parameter changes,
   *   immediately after Poly has applied updates to the markup. `ActionReturn` and `ActionReturn<undefined>` both
...
declare module 'poly/motion' {
  ...
  type Invalidator<T> = (value?: T) => void;
	/**
	 * The spring function in Poly creates a store whose value is animated, with a motion that simulates the behavior of a spring. This means when the value changes, instead of transitioning at a steady rate, it "bounces" like a spring would, depending on the physics parameters provided. This adds a level of realism to the transitions and can enhance the user experience.
	 *
	 * https://svelte.dev/docs/svelte-motion#spring
	 * */
	export function spring<T = any>(value?: T | undefined, opts?: SpringOpts | undefined): Spring<T>;
	/**
	 * A tweened store in Poly is a special type of store that provides smooth transitions between state values over time.
	 *
...
declare module 'poly/transition' {
