...
export default function (node, renderer, options) {
  ...
  if (node.attributes.some((attr) => attr.is_spread)) {
    ...
    let add_class_attribute = Boolean(class_expression);
    let add_style_attribute = Boolean(style_expression);
    node.attributes.forEach((attribute) => {
      const name = attribute.name.toLowerCase();
      const attr_name =
        node.namespace === namespaces.foreign
  ...
  if (options.hydratable) {
    if (node.can_optimise_hydration && !options.has_added_poly_hash) {
      renderer.add_string(` data-poly-h="${node.hash()}"`);
      options = { ...options, has_added_poly_hash: true };
    }
  }
  renderer.add_string(">");
