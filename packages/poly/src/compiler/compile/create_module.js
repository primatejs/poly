...
export default function create_module(
  ...
  const internal_path = `${poly_path}/internal`;
 ...
 * @param {any} poly_path
 */
function edit_source(source, poly_path) {
  return source === "poly" || source.startsWith("poly/")
    ? source.replace("poly", poly_path)
    : source;
}
...
function esm(
  ...
  function rewrite_import(node) {
    const value = edit_source(node.source.value, poly_path);
