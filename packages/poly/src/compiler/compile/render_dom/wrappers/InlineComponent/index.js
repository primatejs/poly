...
export default class InlineComponentWrapper extends Wrapper {
  ...
  render(block, parent_node, parent_nodes) {
    ...
    const uses_spread = Boolean(this.node.attributes.find((a) => a.is_spread));
    // removing empty slot
  ...
  (this.node.attributes.length > 0 || this.node.bindings.length > 0) &&
        ...
        if (${switch_value}) {
          ${name} = @construct_poly_component(${switch_value}, ${switch_props}(#ctx));
      ...
      block.chunks.update.push(b`
        if (${update_condition}) {
          ...
          if (${switch_value}) {
            ${update_insert}
            ${name} = @construct_poly_component(${switch_value}, ${switch_props}(#ctx, #dirty));
