...
export default class ElementWrapper extends Wrapper {
  ...
  constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
    ...
    this.has_dynamic_attribute = Boolean(this.attributes.find(
  ...
  this.renderer.options.dev &&
    ...
    if (is_tag_dynamic) {
      ...
      const has_transitions = Boolean(this.node.intro || this.node.outro);
  ...
  render_element(block, parent_node, parent_nodes) {
    const { renderer } = this;
    const { hydratable } = renderer.options;
      ...
      if (this.fragment.nodes.length === 1 && this.fragment.nodes[0].node.type === "Text") {
        ...
        if (hydratable) {
          block.chunks.claim.push(
            b`if (@get_poly_dataset(${node}) !== "${this.node.hash()}") ${node}.textContent = ${text};`,
          );
        }
      } else {
        const state = {
        ...
        if (hydratable) {
          block.chunks.claim.push(
            b`if (@get_poly_dataset(${node}) !== "${this.node.hash()}") ${node}.${property} = ${literal};`,
          );
        }
      }
    } else {
  ...
  get_claim_statement(block, nodes, to_optimise_hydration) {
    ...
    if (to_optimise_hydration) {
      attributes.push(p`["data-poly-h"]: true`);
    }
