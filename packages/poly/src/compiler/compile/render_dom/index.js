...
export default function dom(component, options) {
  ...
  const should_add_css = Boolean(styles) && (options.customElement || options.css === "injected");
  if (should_add_css) {
    body.push(b`
  ...
  if (has_definition) {
    ...
    component.reactive_declarations.forEach((d) => {
      ...
      const uses_rest_or_props = Boolean(dependencies.find((n) => n === "$$props" || n === "$$restProps"));
  ...
  const superclass = {
    type: "Identifier",
    name: options.dev ? "@PolyComponentDev" : "@PolyComponent",
  };
  ...
  has_create_fragment ? "create_fragment" : "null"
        ...
        ${
  options.dev &&
          b`@dispatch_dev("PolyRegisterComponent", { component: this, tagName: "${name.name}", options, id: create_fragment.name });`
}
      }
    }
  `[0]
  ...
  if (options.discloseVersion === true) {
    component.imports.unshift({
      ...
      source: {
        type: "Literal",
        value: `${options.polyPath ?? "poly"}/internal/disclose-version`,
      },
    });
  }
