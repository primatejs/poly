...
import {
  ...
  extract_poly_ignore_from_comments,
} from "../utils/extract_poly_ignore.js";
...
export default class Component {
  ...
  generate(result) {
    ...
    if (result) {
      ...
      const banner = `${this.file ? `${this.file} ` : ""}generated by Poly v${VERSION}`;
  ...
  extract_exports(node, module_script = false) {
    const ignores = extract_poly_ignore_from_comments(node);
    if (ignores.length) {
    ...
    for (let i = 0; i < body.length; i += 1) {
      ...
      if (node.type === "VariableDeclaration") {
        const all_hoistable = node.declarations.every((d) => {
          ...
          // everything except const values can be changed by e.g. poly devtools
  ...
  warn_if_undefined(name, node, template_scope, owner) {
    ...
    this.warn(node, compiler_warnings.missing_declaration(name, Boolean(this.ast.instance)));
  }
  ...
  const component_options = {
    ...
    accessors:
      "accessors" in component.compile_options
        ...
        : Boolean(component.compile_options.customElement),
    preserveWhitespace: Boolean(component.compile_options.preserveWhitespace),
