...
export class MappedCode {
  ...
  static from_source({ source, file_basename, get_location }) {
    ...
    // we know that it will eventually be merged with poly's map,
...
/**
 * @param {string} filename
 * @param {import('magic-string').SourceMap} poly_map
 ...
 */
export function apply_preprocessor_sourcemap(filename, poly_map, preprocessor_map_input) {
  if (!poly_map || !preprocessor_map_input) {
    return poly_map;
  }
  ...
  const result_map = combine_sourcemaps(filename, [poly_map, preprocessor_map]);
  // Poly expects a SourceMap which includes toUrl and toString. Instead of wrapping our output in a class,
