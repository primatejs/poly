...
const regex_poly_ignore = /^\s*svelte-ignore\s+([\s\S]+)\s*$/m;

/**
 * @param {string} text
 * @returns {string[]}
 */
export function extract_poly_ignore(text) {
  const match = regex_poly_ignore.exec(text);
  return match
    ? match[1]
 ...
 * @returns {string[]}
 */
export function extract_poly_ignore_from_comments(node) {
  return flatten(
    (node.leadingComments || []).map((comment) => extract_poly_ignore(comment.value)),
  );
}
