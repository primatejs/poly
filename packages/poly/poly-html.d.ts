...
// It's meant to be loaded directly by the Poly language server
/* eslint-disable @typescript-eslint/no-empty-interface */

import * as polyElements from './elements.js';

/**
 * @internal do not use
 */
type HTMLProps<Property extends string, Override> = Omit<
	import('./elements.js').PolyHTMLElements[Property],
	keyof Override
> &
	Override;
...
declare global {
	...
	namespace polyHTML {
		...
		function createElement<Elements extends IntrinsicElements, Key extends keyof Elements>(
			...
			attrs: string extends Key ? polyElements.HTMLAttributes<any> : Elements[Key]
		...
		function createElement<Elements extends IntrinsicElements, Key extends keyof Elements, T>(
			...
			element: Key | undefined | null,
			attrsEnhancers: T,
			attrs: (string extends Key ? polyElements.HTMLAttributes<any> : Elements[Key]) & T
		): Key extends keyof ElementTagNameMap
			? ElementTagNameMap[Key]
			: Key extends keyof SVGElementTagNameMap
			? SVGElementTagNameMap[Key]
		...
		// For backwards-compatibility and ease-of-use, in case someone enhanced the typings from import('poly/elements').HTMLAttributes/SVGAttributes
		...
		/**
		 * Avoid using this interface directly. Instead use the `PolyHTMLElements` interface exported by `poly/elements`
		 * This should only be used if you need to extend the interface with custom elements
		 */
		interface IntrinsicElements extends polyElements.PolyHTMLElements {
			...
			// don't type svelte:options, it would override the types in poly/elements and it isn't extendable anyway
